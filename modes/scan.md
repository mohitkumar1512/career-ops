...
Scan configured job portals, filter by title relevance, and add new listings to the pipeline for later evaluation.
...
5. **Level 2 — ATS APIs / feeds** (parallel):
   ...
   d. For **BambooHR**, the list only returns basic metadata. For each relevant item, read `id`, GET `https://{company}.bamboohr.com/careers/{id}/detail`, and extract full JD from `result.jobOpening`. Use `jobOpeningShareUrl` as the public URL if present; otherwise use the detail URL.
   e. For **Workday**, send POST JSON with at least `{"appliedFacets":{},"limit":20,"offset":0,"searchText":""}` and paginate by `offset` until exhausted
...
6b. **Filter by location (optional)** using `location_filter` from `portals.yml`:
   ...
   - Empty location on a listing → passes (do not penalize missing data)
   ...
   - Location is persisted as 7th column in `scan-history.tsv` for later auditing
...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   WebSearch results can be stale (Google caches results for weeks or months). To avoid evaluating expired listings, verify with Playwright each new URL from Level 3. Levels 1 and 2 are inherently real-time and do not require this verification.
   ...
   c. Classify:
      ...
      - **Expired** (any of these signals):
        - Final URL contains `?error=true` (Greenhouse redirects like this when a listing is closed)
...
11. **Expired listings (Level 3)**: record with status `skipped_expired`
...
Fallback: if you only have the direct ATS URL, first navigate to the company website and find its corporate careers page. Use the direct ATS URL only if the company has no corporate careers page.
...
**Known API/feed patterns by platform:**
